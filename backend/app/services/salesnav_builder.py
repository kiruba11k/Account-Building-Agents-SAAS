...
def build_salesnav_company_search(filters):
    ...
    for emp in employee_values:
        ...
        sid = _employee_min_to_size_id(emp)

        if sid and sid not in size_ids:
            size_ids.append(sid)
    ...
    if revenue:
        ...
        rid = REVENUE_MAP.get(revenue)

        if rid:

            filter_parts.append(
                f"(type:COMPANY_REVENUE,values:List((id:{rid},selectionType:INCLUDED)))"
            )
