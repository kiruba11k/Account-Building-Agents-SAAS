...
def _fetch_identity_id_from_agent(agent_id):
    """Fetch default identity id configured on the target agent."""
    if not agent_id:
        return None
    ...
    try:
        ...
        for candidate in candidates:
            for key in ("identityId", "identity", "identity_id"):
                value = candidate.get(key)
                if value:
                    return str(value)
    except Exception as e:
        print("Unable to fetch agent identity:", e)
...
def _build_launch_argument(search_url, runtime_options=None):
    ...
    argument = {
        # Default schema for Sales Navigator Search Export expects searches as string.
        "searches": search_url,
        "numberOfResultsPerLaunch": int(options.get("numberOfResultsPerLaunch", 100)),
    }

    if options.get("use_queries"):
        argument["queries"] = [search_url]
    ...
    identity_id = (
        ...
        or _fetch_identity_id_from_agent(SEARCH_AGENT_ID)
